...
class ToggleButton extends Phaser.GameObjects.Container {
  constructor (config) {
    ...
    Align.scaleToGameW(this.back, 0.05)
    Align.scaleToGameW(this.onIcon, 0.025)
    Align.scaleToGameW(this.offIcon, 0.025)
  ...
  toggle () {
    ...
    this.setIcons()
    if (this.event) {
      window.game.emitter.emit(this.event, this.value)
    }
  }
